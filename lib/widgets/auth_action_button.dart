...
//   Navigator.push(
//       context, MaterialPageRoute(builder: (context) => const SelectWeakness()));
...
Future _signUp(context) async {
  ...
  String? iin = _passwordTextEditingController.text;
  User userToSave = User(
    name: name,
    iin: iin,
    modelData: predictedData,
  ...
  Navigator.push(context,
      MaterialPageRoute(builder: (BuildContext context) => SelectWeakness()));
}
      ...
      onTap: () async {
        try {
          ...
          if (faceDetected) {
            ...
            PersistentBottomSheetController bottomSheetController =
                Scaffold.of(context).showBottomSheet(
                    (context) => signSheet(context, predictedUser));
            bottomSheetController.closed.whenComplete(() => widget.reload());
          }
        } catch (e) {
          // If an error occurs, log the error to the console.
